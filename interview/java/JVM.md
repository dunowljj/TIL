...
- 스택 기반 가상머신
- 바이트 코드를 OS에 특화된 코드로 변환하여 실행한다. 특정 기기나 OS에 종속되지 않는 런타임 환경을 제공한다.
...
#### 2.4.1. Method(Static, Class) Area

- Method Area는 JVM이 실행 중인 애플리케이션의 **클래스 메타데이터(Class Metadata)**를 저장하는 논리적 메모리 영역
- 클래스와 인터페이스에 대한 런타임 상수 풀, 메서드와 필드, static 변수, 메서드 바이트 코드 등을 보관
- JVM 스펙은 Method Area의 필수 역할을 정의하지만, 구현 방식(물리적 위치, 메모리 관리 방식 등)은 JVM 제작자에 따라 다르게 구현되어 다를 수 있다.
  - Java 8 이후의 Oracle HotSpot JVM과 이를 기반으로 한 대부분의 JVM 구현은 Metaspace를 사용

#### Runtime Constant Pool
- 클래스가 JVM에 의해 로드될 때, Class Constant Pool의 데이터를 기반으로 생성
- 링킹 과정의 해석(resolve)단계에서, 이 상수 풀 내의 심볼릭 레퍼런스는 실제 메모리 주소나 참조로 변환
#### Class Constant Pool
- 클래스 파일 내부에 존재하며, 컴파일된 클래스의 모든 상수와 심볼릭 참조를 저장하는 테이블
- 컴파일 시점에 생성되어 `.class` 파일에 포함
...
- JNI(Java Native Interface)를 통해 호출되는 네이티브 메서드의 매개변수, 지역 변수, 중간 계산 데이터 등이 저장된다.
...
- 자바가 아닌 다른 언어로 작성된 코드. 일반적으로 운영 체제의 기능이나 특정 하드웨어 자원에 직접 접근해야 할 때 사용된다.
- 네이티브 코드가 시스템 라이브러리나 외부 네이티브 라이브러리를 직접 호출한다. 그래서 JNI를 사용하여 호출되는 네이티브 코드는 JVM이 관리하는 힙 메모리가 아닌, **운영 체제의 네이티브 메모리 영역**에 적재된다.
- JVM의 가비지 컬렉터의 관리 범위 밖에 있다. 애초에 JVM은 바이트코드 기반이라 네이티브 코드의 실행을 직접 못한다.
- 네이티브 메모리와 네이티브 코드3는 Java 실행과는 별개로 운영 체제나 특정 플랫폼의 기능으로 이미 존재하며, Java에서는 이를 호출하거나 사용할 수 있도록 JNI(Java Native Interface)와 같은 메커니즘을 제공하는 것
